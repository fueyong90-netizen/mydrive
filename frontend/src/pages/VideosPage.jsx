...
import { Video, Loader2, AlertTriangle, Search } from "lucide-react";
    ...
    <div className="py-4 sm:py-8">
      ...
      <header className="mb-8">
        <h1 className="text-3xl font-bold text-gray-100 mb-2 flex items-center">
          <Video className="w-8 h-8 mr-3 text-green-400" />
          Mes Vidéos
        </h1>
        <p className="text-gray-400">Toutes vos vidéos en un seul endroit</p>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <div className="bg-gray-800 p-4 rounded-xl border border-gray-700">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-400">Nombre de vidéos</p>
              <p className="text-3xl font-bold text-green-400 mt-1">{files.length}</p>
            </div>
            <Video className="w-12 h-12 text-green-400 opacity-30" />
          </div>
        </div>
        ...
        <div className="bg-gray-800 p-4 rounded-xl border border-gray-700">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-red-400 mt-1">{formatSize(totalSize)}</p>
            </div>
            <Video className="w-12 h-12 text-red-400 opacity-30" />
          </div>
        </div>
      </div>
      ...
      {!loading && !error && filteredFiles.length === 0 && (
        <div className="bg-gray-900 rounded-xl border border-gray-800 p-10 flex flex-col items-center justify-center text-gray-500">
          ...
          <p className="text-sm mt-2">Uploadez vos premières vidéos dans la section Fichiers</p>
        </div>
      )}
        ...
        <div className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
          <div className="p-4 bg-gray-800 border-b border-gray-700">
            <p className="text-sm text-gray-400">
              Affichage de {filteredFiles.length} vidéo{filteredFiles.length > 1 ? 's' : ''}
            </p>
          </div>
